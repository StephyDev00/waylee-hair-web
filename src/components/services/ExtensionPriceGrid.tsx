...
export function ExtensionPriceGrid({ prices }: { prices: ExtensionPrice[] }) {
  const t = useTranslations("services.extensionGrid");
  ...
  return (
    <div className="rounded-2xl border border-wine/20 bg-ink px-6 py-10 text-ivory md:px-12">
      ...
      <p className="mt-1 text-xs uppercase tracking-[0.3em] text-ivory/50">{t("subtitle")}</p>

      <div className="mt-8 overflow-x-auto">
        <table className="w-full min-w-[420px] border-collapse text-sm">
          <thead>
            <tr className="border-b border-ivory/15 text-left text-xs uppercase tracking-wide text-ivory/50">
              <th className="py-3 pr-4 font-medium">{t("length")}</th>
              {textures.map((tex) => (
                <th key={tex} className="py-3 px-4 font-medium">
                  {textureLabels[tex]}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {lengths.map((length) => (
              <tr key={length} className="border-b border-ivory/10">
                ...
                {textures.map((tex) => (
                  <td key={tex} className="py-3 px-4 text-wine-light">
                    CHF {priceFor(length, tex)}.-
                  </td>
                ))}
              </tr>
            ))}
      ...
      <p className="mt-6 text-xs text-ivory/40">{t("note")}</p>
    </div>
  );
}
